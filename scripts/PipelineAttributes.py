...
class default_attributes:
    ...
    def insert_into_process_attributes(self, conn, process_id, attribute_key, attribute_value):
        ...
        if process_id != "" and attribute_key != "":
            query = "INSERT INTO process_attributes (process_id,attribute_key,attribute_value) values('{}','{}','{}')".format(process_id, attribute_key, attribute_value)
            cursor = conn.cursor()
            try:
    ...
    def get_attribute_value(conn, attribute_key, process_id):
        query = "select attribute_value from process_attributes where attribute_key='{}' and process_id='{}' ".format( attribute_key, process_id)
        cursor = conn.cursor()
        cursor.execute(query)
...
class stages:
    ...
    def insert_into_process_stages(self, conn, process_id, selection_id, stage_name):
        ...
        query = "INSERT INTO process_stages (process_id,selection_id,stage_name) values('{}','{}','{}')".format(process_id, selection_id, stage_name)
        cursor = conn.cursor()
        try:
            cursor.execute(query)
            conn.commit()
    ...
    def check_started(self, conn):
        query = "select distinct process_id from process_stages where stage_start is null and stage_name='{}'".format(self.stage_list)
        cursor = conn.cursor()
        cursor.execute(query)
        process_id_all = list()
        for row in cursor:
    ...
    def set_started(self, conn):
        query = "update process_stages set stage_start=NOW() where process_id='{}' and stage_name='{}'".format(self.process_id, self.stage_list)
        cursor = conn.cursor()
        try:
            cursor.execute(query)
            conn.commit()
    ...
    def set_finished(self, conn):
        query = "update process_stages set stage_end=NOW() where process_id='{}' and stage_name='{}'".format(self.process_id, self.stage_list)
        cursor = conn.cursor()
        try:
            cursor.execute(query)
            conn.commit()
