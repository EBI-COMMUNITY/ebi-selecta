...
class bsub(object):
    ...
    @property
    def command(self):
        return self._kwargs_to_flag_string() + (('  "{}"'.format(self.args[0])) if len(self.args) else "")
    ...
    def running_jobs(self):
        ...
        """
        results=[]
        bjobs = sp.check_output(["bjobs"], shell=True)
        print(bjobs)
        bjobs = bjobs.rstrip().decode()
        print(bjobs)
        for x in bjobs.split("\n")[1:]:
            print(x)
